function [epoched_data] = epoch_hyperscanning_data_full_trial(set_name, onset_marker, offset_marker)
   
    % Load the EEG dataset
    datapath = 'C:\Users\mdomin23\OneDrive - McGill University\Study Materials\3_processing\ready_data_v0';
    ...
    unique_labels = unique(codelabels(contains(codelabels, "Speaker_A") | contains(codelabels, "Speaker_B")));
    drop_substrings = [{'T1'}, {'boundary'}, unique_labels(:)'];
...
%         matches = contains(codelabels, substring);
    ...
    % Misc Prep
%     suffix_length = length(onset_marker) + 1;
%     new_codelabels = {EEG.event.codelabel};
